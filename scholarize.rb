...
QUI = File.dirname(File.realpath(__FILE__))
puts QUI

def parse_file_names(globs)
  globs
    .map { |a| Dir.glob a }
    .flatten
    .map { |a| File.realpath a }
    .uniq 
end

file_paths = parse_file_names(ARGV)

file_paths.each do |path|
  pt = File.basename(path).split('.')[0...-1].join('.')
  extension = File.basename(path).split('.').last
  pt = I18n.transliterate pt
  pt = pt.gsub(/\([^()]*\)/, '')
  pt = pt.gsub(/[^a-z0-9]/i,' ')
  pt = pt.gsub(/\b[A-Z]\b/i, '')
  pt = pt.squeeze(' ').strip.downcase
  puts pt
  ...
  if !result.empty?
    ...
    puts safe_authors
    puts safe_title
    safe_filename = "#{safe_title}_#{safe_authors}.#{extension}"
    puts "Proposed filename:\n  ".green + safe_filename.green
    File.rename path, File.join(File.dirname(path), safe_filename)
  else
    puts "Problems, mate".red
    FileUtils.mkdir_p File.join(QUI, 'muddlers')
    new_path = File.join(QUI, 'muddlers', File.basename(path))
